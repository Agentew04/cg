...
Vector3 Vector3::rotate(float angle, const Vector3 &axis) const
{
   ...
   auto point = *this;
   float dot = ax.dot(point);
   Vector3 cross = ax.cross(point);
   float x = point.x * cos(angle) + cross.x * sin(angle) + ax.x * dot * (1 - cos(angle));
   float y = point.y * cos(angle) + cross.y * sin(angle) + ax.y * dot * (1 - cos(angle));
   float z = point.z * cos(angle) + cross.z * sin(angle) + ax.z * dot * (1 - cos(angle));
